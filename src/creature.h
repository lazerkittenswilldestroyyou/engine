...
class Creature : virtual public Thing
{
	...
	protected:
		...
		//Xor
		double m_stun = 1.0;
		double m_stun_tick = 0.1;
		double getStun() { return m_stun; }
		void refreshStun() { m_stun = 1; }
		void tickStun() {
			if (m_stun > 1.101) { m_stun = 1; }
			else m_stun = m_stun - m_stun_tick;
		}
		void gainStun(double gain_stun) { m_stun = m_stun + gain_stun; }
